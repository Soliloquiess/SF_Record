...
public class G3_AdjListTest2 {
	...
	//G2에서 노드 다 지움(노드가 필요 없으니까.

	static int N;
	static ArrayList<Integer>[] adjList;	//어레이리스트 배열 뒤에 [] 붙여야
	//정점의 번호만 넣을거라 제네릭은 Integer라 주자.
	...
	public static void main(String[] args) throws NumberFormatException, IOException {
		...
		adjList = new ArrayList[N];	//정점의 개수만큼 배열 만듬

		for(int i =0; i<N;i++) {
			adjList[i] = new ArrayList<Integer>();
		}
		
		//인접리스트는 어레이리스트니까 어레이배열로 주자
		...
		for(int i =0; i<C;i++) {
			...
			adjList[to].add(from);	//뒤집어서 한번 더해야
...
//			그래서 인접여부 체크 할 필요가 없다.
			...
			for(int temp : adjList[current]) { //temp: current와 인접정점인 해당정점의 번호
				...
				//Node temp = adjList[current]; temp!=null; temp = temp.next 
				//부분은 어레이리스트 인접부분부터 쭉 바뀌면 됨. 젤 쉬운건 foreach로 돌리자.
...
//탐색하는 법이 바뀌는 거 bfs 논리는 그대로임. 이 아래는 인접 체크 조건이 바뀐거. 불필요한 조건이 없어짐.
					...
					if(!visited[temp]) {	//연결은 되어있으니 인접여부는 판단할 필요가 없고  노드가 저장된 정점을 보면 그게 어떤 정점 정보인지 알 수 있다. 그게 방문되었는지 아닌지만 체크
						queue.offer(temp);
						visited[temp] = true;
						//vertex가 정점번호였으니까 빼주자.
					}
