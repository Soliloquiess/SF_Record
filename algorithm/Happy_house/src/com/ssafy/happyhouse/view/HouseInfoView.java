...
public class HouseInfoView {
	...
	ReadCSVTest csv = ReadCSVTest.getInstance();
	...
	private void showHouseInfo(int code) {
		...
		// } else if(curHouse.getAptName() != null &&
		// curHouse.getAptName().trim().length() != 0) {
		// icon = new ImageIcon("img/" + curHouse.getAptName());
		//
		// if(icon == null) {
		...
		Image img = null;
		try {
			img = ImageIO.read(new File("img/" + curHouse.getAptName()));
	...
	public void setMain() {
		...
		sortDong = new JButton("정렬(동)");
		sortPrice = new JButton("정렬(거래금액)");
		rightTop2.add(findC);
		rightTop2.add(wordTf);
		rightTop2.add(searchBt);
		rightTop2.add(sortDong);
		rightTop2.add(sortPrice);
		...
		// 바꾸세요.
		searchBt.addActionListener(e -> searchHouses(0));
		sortDong.addActionListener(e -> {
			searchHouses(1);
		});
		sortPrice.addActionListener(e -> {
			searchHouses(2);
		});
	...
	/** 검색 조건에 맞는 주택 정보 검색 */
	private void searchHouses(int flag) {
	 ...
	 * 주택 목록을 갱신하기 위한 메서드
	 */
	public void showHouses(int flag) {
		...
		if (flag == 1) {
			deals.sort((a, b) -> a.getDong().compareTo(b.getDong()));
		}
		if (flag == 2) {
		...
		if (deals != null) {
			...
			for (HouseDeal deal : deals) {
				...
				if (deal.getDealAmount() == (flag == 2 ? "0" : null)) {
					data[i][3] = "없음";
				} else {
					data[i][3] = deal.getDealAmount().trim();
				}

				data[i++][4] = deal.getType();
				// csv.search(deal.getDong());
			}
